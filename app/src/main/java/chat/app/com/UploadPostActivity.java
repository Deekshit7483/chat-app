...
public class UploadPostActivity extends AppCompatActivity {
	...
	private StorageReference images = _firebase_storage.getReference("image");
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		switch1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton _param1, boolean _param2) {
				...
				if (_isChecked) {
					switch1.setText("Comment on");
				}
				else {
					switch1.setText("Comment off");
				}
			}
		});
		...
		textview1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				data.edit().putString("mention", "true").commit();
				intent.setClass(getApplicationContext(), MentionActivity.class);
		...
		_fab.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				key = post.push().getKey();
				if (photo.equals("")) {
					
				}
				else {
					post_.child(Uri.parse(photo).getLastPathSegment()).putFile(Uri.fromFile(new File(photo))).addOnFailureListener(_post__failure_listener).addOnProgressListener(_post__upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
						@Override
						public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
							return post_.child(Uri.parse(photo).getLastPathSegment()).getDownloadUrl();
						}}).addOnCompleteListener(_post__upload_success_listener);
					_fabRotation(true);
				}
			}
		});
		...
		_post__upload_success_listener = new OnCompleteListener<Uri>() {
			...
			public void onComplete(Task<Uri> _param1) {
				...
				map.put("post name", Uri.parse(photo).getLastPathSegment());
				map.put("comment", switch1.getText().toString());
				map.put("time", String.valueOf((long)(from.getTimeInMillis())));
				map.put("push key", key);
				if (!data.getString("mention_information", "").equals("")) {
					map.put("mention", new Gson().toJson(appInfo));
				}
				post.child(key).updateChildren(map);
				SketchwareUtil.showMessage(getApplicationContext(), "Post sent");
				_fabRotation(false);
	...
	private void initializeLogic() {
		...
		if (data.getString("mention_information", "").equals("")) {
			...
			appInfo = new Gson().fromJson(data.getString("mention_information", ""), new TypeToken<HashMap<String, Object>>(){}.getType());
			textview2.setText(" @".concat(appInfo.get("name").toString()));
		}
	}
